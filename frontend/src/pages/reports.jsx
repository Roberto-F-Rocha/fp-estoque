...
import { ReportAuditPage } from "./report_audit.jsx";
...
const SUMMARY_LABELS = {
  ...
  end_inventory_value: "Valor do estoque ao final",
...
function summaryLabel(key) {
  return SUMMARY_LABELS[key] || String(key || "").replaceAll("_", " ");
}
...
export function ReportsPage({ notify }) {
  ...
  return (
    <>
      <div className="report-tabs" role="tablist" aria-label="Seções de relatórios">
        <button
          type="button"
          role="tab"
        ...
        >
          ...
          <span>
            ...
            <small>Visualize e exporte em PDF ou Excel.</small>
          </span>
        </button>
        <button
          type="button"
        ...
        >
          <ShieldCheck size={18} />
          <span>
            <strong>Ouvidoria de relatórios</strong>
            <small>Consulte todas as solicitações e abra os arquivos.</small>
          </span>
        </button>
      </div>

      {activeTab === "audit" ? (
        <ReportAuditPage notify={notify} />
      ) : (
        <>
          <PageHeader
            actions={
                  ...
                  columns={
                    preview.report_type === "daily_movements"
                      ? [
                        ...
                        { key: "lot", label: "Lote" },
                        { key: "previous", label: "Quantidade anterior" },
                        { key: "quantity", label: "Quantidade movimentada" },
                        { key: "final", label: "Quantidade final" },
